...
from consts import BATCH_SIZE, EPOCHES, LR, HIDDEN, LIMIT
...
class GCN(Module):
    def __init__(self, hidden_channels, count=2):
        super(GCN, self).__init__()
        self.conv_in = GCNConv(2, hidden_channels)
...
def train(dataset_raw):
    ...
    dataset = []
    # Hitting CUDA OOM issues if this isn't constrained.
    for graph, score in tqdm(dataset_raw[:LIMIT]):
    ...
    for epoch in range(0, int(EPOCHES*0.1)):
        train()
    ...
    for epoch in range(0, int(EPOCHES*0.9)):
        train()
        train_acc = test(train_loader)
        test_acc = test(test_loader)
        print(
